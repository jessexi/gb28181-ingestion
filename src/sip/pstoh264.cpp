...
int PsToH264::GetH246FromPs(char *buffer, int length, char **h264Buffer, int *h264length)
{
    ...
    *h264length = 0;
    ...
    while (leftlength >= sizeof(pack_start_code))
    {
        ...
        if (/*m_firstMainFrame && */ NextPack && NextPack[0] == '\x00' && NextPack[1] == '\x00' && NextPack[2] == '\x01' && NextPack[3] == '\xE0')
        {
        ...
        {
            if (length < sizeof(pack_title_header))
                break;
        ...
        {
            if (leftlength < sizeof(program_stream_map))
                break;
